...
//  CANCEL ORDER OR SPECIFIC PRODUCTS
export const cancelOrder = async (req, res) => {
  try {
    ...
    if (itemId) {
      ...
      if (["Cancelled", "Returned"].includes(item.orderStatus)) {
    ...
    await order.save();
    return res.status(200).json({ success: true, message: "Cancellation processed successfully." });
  } catch (error) {
...
//  RETURN ORDER OR SPECIFIC PRODUCTS
export const returnOrder = async (req, res) => {
  try {
    ...
    if (itemId) {
      ...
      const allReturned = order.products.every(p => ["Returned", "Cancelled"].includes(p.orderStatus));
      if (allReturned) {
        order.orderStatus = "Returned"; // Mark main order as Returned
      }
    } else {
      // Return entire order
    ...
    await order.save();
    return res.status(200).json({ success: true, message: "Return request processed successfully." });
  } catch (error) {
